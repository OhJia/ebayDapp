...
contract EcommerceStore {
  ...
  function getProductAuctionInfo(uint _productIndex) view public returns (uint, address, uint, uint, uint, ProductStatus, ProductCondition) {

    Product memory product = stores[productIdInStore[_productIndex]][_productIndex];
    
    return (product.startPrice, product.highestBidder, product.highestBid, product.secondHighestBid, product.totalBids, product.status, product.condition);
  }
  ...
  function revealBid(uint _productId, string _amount, string _secret) public {
    ...
    /* check at there is a bid that match with reveal */
    require (bidInfo.bidder > 0);
    /* and that it hasn't been revealed already */
    ...
    if (bidInfo.value < amount) {
      /* if sent Ether is less than bid amount, they lost: refund */
      refund = bidInfo.value;
    } else {
      /* if first to reveal as highest bidder */
      /* TODO: why need address()? */
  ...
  function escrowInfo(uint _productId) view public returns (address, address, address, uint, bool, uint, uint) {
